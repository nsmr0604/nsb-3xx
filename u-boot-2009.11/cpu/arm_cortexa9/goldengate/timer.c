...
static unsigned long timer_clock = 0;
...
void reset_timer_masked (void)
{
	...
	lastdec = READ_TIMER / (timer_clock / CONFIG_SYS_HZ);
...
ulong get_timer_masked (void)
{
	...
	ulong now = READ_TIMER / (timer_clock / CONFIG_SYS_HZ);

	if (lastdec < now)    /* count down timer underflow */
	    /* timestamp+= TIMER_LOAD_VAL/(CONFIG_TIMER_CLOCK/CONFIG_SYS_HZ)
	    			-now+lastdec; */
