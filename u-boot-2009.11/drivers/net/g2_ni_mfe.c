...
#if defined(CONFIG_CS75XX_GMAC1_TO_EXT_SWITCH) && \
		(CONFIG_UBOOT_CPU_TO_GMAC == 1)
	/*
...
int g2_eth_initialize(bd_t *bis)
{
	...
	/* advertisement 10M full duplex, pause capable on */
	g2_phy_write(GE_PORT_PHY_ADDR,0x04,0x461);
	/* Enable and Restart Auto-Negotiation */
    	g2_phy_write(GE_PORT_PHY_ADDR,0x00,0x1200);

	for (i=0; i<PHY_POLL_TIMES; i++)
	{
		val = g2_phy_read(GE_PORT_PHY_ADDR,0x01);
		if ((val & 0x04)==0x04)
			break;
		udelay(10000);
	}
	...
	for (i=0; i<10; i++)
	{
		val = g2_phy_read(GE_PORT_PHY_ADDR,0x04);
		if (val>0) {
			break;
		}
		udelay(1000);
	...
	val = (val & 0x05E0) >> 5;
	if ((val & 0x08)==0x08) /* 100M full duplex */
		printf(" 100M/Full \n");
    	else if ((val & 0x04)==0x04) /* 100M half duplex */
		printf(" 100M/Half \n");
    	else if ((val & 0x02)==0x02) /* 10M full duplex */
		printf(" 10M/Full \n");
	else if ((val & 0x01)==0x01) /* 10M half duplex */
		printf(" 100M/Half \n");
#endif
