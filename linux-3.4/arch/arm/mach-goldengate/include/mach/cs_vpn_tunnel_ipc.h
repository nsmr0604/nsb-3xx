...
typedef enum {
	...
	CS_FUNC_ID_464XLAT = 12,
	CS_FUNC_ID_L2TP_IPSEC_MAPE = 14,
	CS_FUNC_ID_MAX = 13,
} cs_vpn_ofld_func_id;
...
#define CS_ETHERIP_IPC_PE_SET_ENTRY_ACK		CS_ETHERIP_IPC_PE_SET_ENTRY
#define CS_ETHERIP_IPC_PE_DEL_ENTRY_ACK		CS_ETHERIP_IPC_PE_DEL_ENTRY
...
typedef struct {
	...
	/* Inner L2 header */
	cs_uint8_t	replace_inner_da;
	cs_uint8_t	replace_inner_sa;
	cs_uint8_t	inner_dst_mac[6];
	cs_uint8_t	inner_src_mac[6];

	/* L3 layer for outer IP header*/
	cs_uint8_t	default_tos;
	cs_uint8_t	hop_limit;

	cs_uint32_t	flow_label;

	cs_l3_ip_addr	tunnel_saddr;      /* Outer IP addr */
	cs_l3_ip_addr	tunnel_daddr;      /* Outer IP addr */

	/* Outer L2 header */
	cs_uint8_t	outer_dst_mac[6];
	cs_uint8_t	outer_src_mac[6];
} __attribute__((__packed__)) g2_ipc_pe_etherip_set_entry_t;

/* ---------------------------------------------------------------
 * CS_ETHERIP_IPC_PE_SET_ENTRY_ACK
 */
typedef struct {
	cs_uint16_t	sa_idx;  /* L2TP sa IDX */
} __attribute__((__packed__)) g2_ipc_pe_etherip_set_entry_ack_t;
...
typedef struct {
	cs_uint16_t	sa_idx;  /* L2TP sa IDX */
} __attribute__((__packed__)) g2_ipc_pe_etherip_del_entry_t;

/* ---------------------------------------------------------------
 * CS_ETHERIP_IPC_PE_DEL_ENTRY_ACK
 */
typedef struct {
	cs_uint16_t	sa_idx;  /* L2TP sa IDX */
} __attribute__((__packed__)) g2_ipc_pe_etherip_del_entry_ack_t;
