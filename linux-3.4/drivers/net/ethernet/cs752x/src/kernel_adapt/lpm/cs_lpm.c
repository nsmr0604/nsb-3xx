...
cs_status_t cs_fwd_result_add(cs_dev_id_t device_id, cs_fwd_result_t *fwd_result, cs_uint32_t *index)
{
	...
	if(fwd_result->sw_action_id != 0) {
		p_fwd_rslt_entry->l3.ip_sa_replace_en = 0;
		p_fwd_rslt_entry->l3.ip_da_replace_en = 0;
		p_fwd_rslt_entry->l3.ip_sa_index = fwd_result->sw_action_id & 0x0fff;
		p_fwd_rslt_entry->l3.ip_da_index = (fwd_result->sw_action_id >> 12) & 0x00ff;
	}
