...
__u8 igmp_hm_idx = ~(0x0);	/* for IGMP packet */
...
__u8 icmpv6_hm_idx = ~(0x0);	/* for ICMPv6 packet */

unsigned int rootport_fwdrslt_idx = ~(0x0), rootport_voqpol_idx = ~(0x0);
__u8 mcast_without_src_hm_idx = ~(0x0); /* for Multicast stream when source IP is not assigned; source filtering is not used */
__u8 mcast_with_src_hm_idx = ~(0x0); /* for Multicast stream when source IP is assigned; source filtering is used */
__u8 mcast_to_dest_hm_idx = ~(0x0); /* for replicated Multicast stream */
