			  ...
			  unsigned char ip_ver, unsigned char dir)
{
	...
	if (state == CS_TUNNEL_ACCELERATED /* HW tunnel exists */) {
		...
		/* Since cs_forward_check_output_ip_protocol() won't store SPI
		 * to cs_cb for NAT-T case, we need to do it here.
		 */
		if (dir == 0 /* outbound */) {
			cs_cb->output.vpn_h.ah_esp.spi = x->id.spi;
		} else {
			/* inbound */
			cs_cb->input.vpn_h.ah_esp.spi = x->id.spi;
		}
