	...
	cs_rule_hash_t  *p_rule_hash)
{
	...
	switch (p_tunnel_cfg->type) {
		...
		case CS_ETHERIP_IPSEC:
				...
				sw_action_id = sa_id & 0x0fff;
				sw_action_id |= ((CS_ETHERIP_IPSEC << 12) & 0xf000);
				p_rule_hash->fwd_result.l3.ip_sa_replace_en = 0;
				p_rule_hash->fwd_result.l3.ip_da_replace_en = 0;
				p_rule_hash->fwd_result.l3.ip_sa_index = sw_action_id & 0x0fff;
				p_rule_hash->fwd_result.l3.ip_da_index = (sw_action_id >> 12) & 0x00ff;
				DBG(printk("%s:%d ip_sa_index=0x%x ip_da_index=0x%x\n", __func__, __LINE__,
						p_rule_hash->fwd_result.l3.ip_sa_index,	p_rule_hash->fwd_result.l3.ip_da_index));
			break;
		default:
