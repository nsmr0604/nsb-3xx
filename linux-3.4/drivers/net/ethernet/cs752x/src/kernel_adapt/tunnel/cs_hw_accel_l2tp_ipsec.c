...
static cs_status_t cs_etherip_add_ipc_send(
	CS_IN cs_tunnel_entry_t				*t)
{
	cs_ipsec_policy_node_t p_node;
	cs_uint8_t protocol;
	...
	if (t->tunnel_cfg.type == CS_ETHERIP_IPSEC ) {
		if (t->tunnel_cfg.tunnel.etherip.ipsec_policy == 0) {
			ERR(printk("%s:%d invalid ipsec_policy\n",
						__func__, __LINE__));
			return CS_E_CONFLICT;
		}
		...
		ret = cs_ipsec_policy_node_get(t->device_id,
				dir,
				t->tunnel_cfg.tunnel.etherip.ipsec_policy,
				&p_node);
		if (ret != CS_OK) {
			ERR(printk("%s:%d can't get IPSec policy and sa\n",
						__func__, __LINE__));
...
#ifdef CS_IPC_ENABLED
		cs_etherip_ipc_send_set_entry(dir, &t->tunnel_cfg, t->sa_id);
#endif /* CS_IPC_ENABLED */
		/* create hash */
		/*cs_l2tp_ipsec_tunnel_hash_create(t);*/
	...
	CS_OUT	cs_uint16_t				*sa_id)
{
	...
	if ((nexthop->nhid.nhop_type != CS_L3_NEXTHOP_TUNNEL_L2TP_IPSEC) &&
		...
		(nexthop->nhid.nhop_type != CS_L3_NEXTHOP_TUNNEL_ETHERIP_IPSEC)) {
	...
	while (t) {
		...
		if (t->tunnel_id == nexthop->id.tunnel_id) {
			if(nexthop->nhid.nhop_type != CS_L3_NEXTHOP_TUNNEL_ETHERIP_IPSEC) {
				/* hit */
	...
	CS_OUT cs_tunnel_id_t				*p_tunnel_id)
{
	...
	*p_tunnel_id = tunnel_node->tunnel_id;
	...
	case CS_L2TPV3_IPSEC:
		...
		break;
	case CS_ETHERIP_IPSEC:
		policy_handle_p =
			&p_tunnel_cfg->tunnel.etherip.ipsec_policy;
		break;
	default:
	...
	/* type = CS_IPSEC or CS_L2TP_IPSEC or CS_ETHERIP_IPSEC */
	if (policy_handle_p) {
...
#ifdef CS_IPC_ENABLED
		/* send IPCs and create hash */
		cs_etherip_add_ipc_send(tunnel_node);
#endif /* CS_IPC_ENABLED */
		break;
	default:
		break;
		...
		)
{
	...
	case CS_ETHERIP_IPSEC:
		policy_handle_p =
			&tunnel_entry->tunnel_cfg.tunnel.etherip.ipsec_policy;
		break;
	default:
		break;
	}
	if (policy_handle_p) {	/* CS_IPSEC or CS_L2TP_IPSEC or CS_ETHERIP_IPSEC*/
		if (*policy_handle_p > 0) {
			/* get IPSec policy node */
			ret = cs_ipsec_policy_node_get(device_id,
					dir,
	...
	case CS_L2TPV3_IPSEC:
		if (p_node.previous_sa_handle > 0) {
			...
			/* send IPCs */
#ifdef CS_IPC_ENABLED
			if (type == CS_L2TP_IPSEC || type == CS_L2TPV3_IPSEC || type == CS_ETHERIP_IPSEC )
...
/*CS_ETHERIP_IPC_PE_ADD_ENTRY*/
cs_status_t
cs_etherip_ipc_send_set_entry(
		cs_sa_id_direction_t direction,
		cs_tunnel_cfg_t *p_tunnel_cfg,
		cs_uint16 sa_id
		)
{
	g2_ipc_pe_etherip_set_entry_t 	etherip_entry_msg;
	cs_etherip_tunnel_cfg_t *p_etherip_cfg;
	cs_uint32_t crc32 = 0;
	if (p_tunnel_cfg == NULL) {
		ERR(printk("%s:%d p_tunnel_cfg NULL pointer\n",
			__func__, __LINE__));
		return CS_E_NULL_PTR;
	}

	p_etherip_cfg = &p_tunnel_cfg->tunnel.etherip;
	if (p_etherip_cfg == NULL) {
		ERR(printk("%s:%d p_etherip_cfg NULL pointer\n",
			__func__, __LINE__));
		return CS_E_NULL_PTR;
	}

	DBG(printk("%s:%d direction = %d\n", __func__, __LINE__, direction));
	memset(&etherip_entry_msg, 0, sizeof(g2_ipc_pe_etherip_set_entry_t));

	/*Fill etherip information*/
	etherip_entry_msg.sa_idx = sa_id;
	etherip_entry_msg.replace_inner_da = p_etherip_cfg->replace_inner_da;
	etherip_entry_msg.replace_inner_sa = p_etherip_cfg->replace_inner_sa;
	/*memcpy(etherip_entry_msg.inner_dst_mac, p_etherip_cfg->inner_dst_mac, CS_ETH_ADDR_LEN);*/
	/*memcpy(etherip_entry_msg.inner_src_mac, p_etherip_cfg->inner_src_mac, CS_ETH_ADDR_LEN);*/
	etherip_entry_msg.default_tos = p_etherip_cfg->default_tos;
	etherip_entry_msg.hop_limit = p_etherip_cfg->hop_limit;
	etherip_entry_msg.flow_label = p_etherip_cfg->flow_label;

	memcpy(&etherip_entry_msg.tunnel_saddr, &p_etherip_cfg->tun_saddr, sizeof(cs_l3_ip_addr));
	memcpy(&etherip_entry_msg.tunnel_daddr, &p_etherip_cfg->tun_daddr, sizeof(cs_l3_ip_addr));

	if(p_tunnel_cfg->dir == CS_TUNNEL_DIR_INBOUND) {
		memcpy(etherip_entry_msg.outer_dst_mac, p_etherip_cfg->outer_dst_mac, CS_ETH_ADDR_LEN);
		memcpy(etherip_entry_msg.outer_src_mac, p_etherip_cfg->outer_src_mac, CS_ETH_ADDR_LEN);
	} else { /* CS_TUNNEL_DIR_OUTBOUND  */
		memcpy(etherip_entry_msg.outer_dst_mac, p_etherip_cfg->local_lan_mac, CS_ETH_ADDR_LEN);
		/* fill the source MAC with the host order */
		etherip_entry_msg.outer_src_mac[1] = CS_ETHERIP_IPSEC & 0xFF;
		etherip_entry_msg.outer_src_mac[2] = 0; /* pkt_type: 0 : LAN->PE,  1: CPU->PE */
		etherip_entry_msg.outer_src_mac[3] = sa_id & 0xFF;
		etherip_entry_msg.outer_src_mac[4] = (p_tunnel_cfg->dir == CS_TUNNEL_DIR_OUTBOUND) ? 0 : 1;
		etherip_entry_msg.outer_src_mac[5] = 0; /* reserved */
		/* Calculate CRC over the MAC SA */
		crc32 = ~(calc_crc(~0, &etherip_entry_msg.outer_src_mac[1], 5));
		/* Store 8 bits of the crc in the src MAC */
		etherip_entry_msg.outer_src_mac[0] = crc32 & 0xFF;
	}
	DBG(printk("%s:%d sa_idx = %d\n", __func__, __LINE__, etherip_entry_msg.sa_idx));
	DBG(printk("%s:%d replace_inner_da = %d\n", __func__, __LINE__, etherip_entry_msg.replace_inner_da));
	DBG(printk("%s:%d replace_inner_sa = %d\n", __func__, __LINE__, etherip_entry_msg.replace_inner_sa));
	DBG(printk("%s:%d default_tos = 0x%x\n", __func__, __LINE__, etherip_entry_msg.default_tos));
	DBG(printk("%s:%d hop_limit = 0x%x\n", __func__, __LINE__, etherip_entry_msg.hop_limit));
	DBG(printk("%s:%d flow_label = 0x%x\n", __func__, __LINE__, etherip_entry_msg.flow_label));

	DBG(printk("tunnel_saddr dump:\n"));
	DBG(cs_dump_data(&etherip_entry_msg.tunnel_saddr, sizeof(cs_l3_ip_addr)));
	DBG(printk("\n"));
	DBG(printk("tunnel_daddr dump:\n"));
	DBG(cs_dump_data(&etherip_entry_msg.tunnel_daddr, sizeof(cs_l3_ip_addr)));
	DBG(printk("\n"));
	DBG(printk("inner_dst_mac dump:\n"));
	DBG(cs_dump_data(&etherip_entry_msg.inner_dst_mac, 6));
	DBG(printk("\n"));
	DBG(printk("inner_src_mac dump:\n"));
	DBG(cs_dump_data(&etherip_entry_msg.inner_src_mac, 6));
	DBG(printk("\n"));
	DBG(printk("outer_dst_mac dump:\n"));
	DBG(cs_dump_data(&etherip_entry_msg.outer_dst_mac, 6));
	DBG(printk("\n"));
	DBG(printk("outer_src_mac dump:\n"));
	DBG(cs_dump_data(&etherip_entry_msg.outer_src_mac, 6));
	DBG(printk("\n"));

	return cs_tunnel_ipc_send(direction, CS_ETHERIP_IPC_PE_SET_ENTRY, &etherip_entry_msg,
					sizeof(g2_ipc_pe_etherip_set_entry_t));
}


/*CS_ETHERIP_IPC_PE_DEL_ENTRY*/
cs_status_t
cs_etherip_ipc_send_del_entry(
		cs_sa_id_direction_t direction,
		cs_uint16 sa_id
		)
{
	g2_ipc_pe_etherip_del_entry_t etherip_entry_msg;

	DBG(printk("%s:%d direction = %d\n", __func__, __LINE__, direction));
	DBG(printk("%s:%d sa_id = %d\n", __func__, __LINE__, sa_id));

	memset(&etherip_entry_msg, 0, sizeof(g2_ipc_pe_etherip_del_entry_t));

	etherip_entry_msg.sa_idx = sa_id;

	return cs_tunnel_ipc_send(direction, CS_ETHERIP_IPC_PE_DEL_ENTRY, &etherip_entry_msg,
					sizeof(g2_ipc_pe_etherip_del_entry_t));
}

cs_status_t cs_etherip_ipc_rcv_set_entry_ack(struct ipc_addr peer,
			      unsigned short msg_no, const void *msg_data,
			      unsigned short msg_size,
			      struct ipc_context *context)
{
	g2_ipc_pe_etherip_set_entry_ack_t *msg;
	cs_tunnel_entry_t *t, t_node;
	cs_uint16_t sa_id;
	cs_status_t ret;
	...
	t = &t_node;
	msg = (g2_ipc_pe_etherip_set_entry_ack_t * )msg_data;
	sa_id = msg->sa_idx;

	DBG(printk("%s:%d\n", __func__, __LINE__));
	DBG(printk("<ETHERIP>: Receive set entry ack from PE%d\n", peer.cpu_id - 1));
	DBG(printk("%s:%d sa_id = %d\n", __func__, __LINE__, sa_id));
	...
	case CS_ETHERIP_IPSEC:
		...
		if(ret != CS_OK) {
			ERR(printk("%s:%d Can't create hash for etherip ipsec tunnel.\n",
				__func__, __LINE__));
			return ret;
		}
		break;
...
cs_status_t cs_etherip_ipc_rcv_del_entry_ack(struct ipc_addr peer,
			      unsigned short msg_no, const void *msg_data,
			      unsigned short msg_size,
			      struct ipc_context *context)
{
	g2_ipc_pe_etherip_del_entry_ack_t *msg;
	cs_uint16_t sa_id;
	cs_uint16_t ofld_seq;
	...
	msg = (g2_ipc_pe_etherip_del_entry_ack_t * )msg_data;
	sa_id = msg->sa_idx;
	/* ofld_seq = msg->ofld_seq; */

	DBG(printk("%s:%d\n", __func__, __LINE__));
	DBG(printk("<ETHERIP>: Receive del entry ack from PE%d", peer.cpu_id - 1));
	DBG(printk("%s:%d sa_id = %d\n", __func__, __LINE__, sa_id));
	/* printk("%s:%d ##### ofld_seq = %d #####\n", __func__, __LINE__, ofld_seq); */
