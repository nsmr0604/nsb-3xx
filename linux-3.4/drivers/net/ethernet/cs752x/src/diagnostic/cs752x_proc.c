...
#define WFO_PE0_FW_VER		"pe0_fw_ver"
#define WFO_PE1_FW_VER		"pe1_fw_ver"
...
/* file handler for WFO_PE0_FW_VER */
static int cs_wfo_get_pe0_fw_ver_read_proc(char *buf, char **start, off_t offset,
				   int count, int *eof, void *data)
{
	u32 len = 0;
	unsigned char *pver_str = GOLDENGATE_RCPU_RRAM1_BASE + 0x150;
	
	if(pver_str[0]=='B' && pver_str[1]=='C' && pver_str[2]=='M'){
		pver_str += 4;	// skip "BCM:"
		printk("BCM-WFO Version:%s\n",pver_str);
	}
	*eof = 1;
	return len;
}
...
//++ For WFO
	...
	cs752x_add_proc_handler(WFO_PE1_FWD_TBL, cs_wfo_get_pe1_fwd_tbl_read_proc,
				NULL,
				proc_driver_cs752x_wfo);
	cs752x_add_proc_handler(WFO_PE0_FW_VER, cs_wfo_get_pe0_fw_ver_read_proc,
				NULL,
				proc_driver_cs752x_wfo);			
