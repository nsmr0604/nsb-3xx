...
static int cs75xx_spi_txrx_msg(struct cs75xx_spi *controller, struct spi_device *slave,
		void *dout, int bitout, void *din, int bitin, u8 t_bits_per_word)
{
	...
	unsigned char	*buf8;
	unsigned int	value;
	unsigned int 	i, wsize, len;

	len = (bitout >= bitin) ? bitout:bitin;
	len /= 8;
	wsize = t_bits_per_word/8;
	...
	list_for_each_entry_safe(t, tmp_t, &m->transfers, transfer_list) {
		...
		if (t->cs_change && !last) {
			status = cs75xx_spi_txrx_msg(controller,
				m->spi, tx_buf,	tx_bits, rx_buf, rx_bits, t_bits_per_word);
		}
	}
	status = cs75xx_spi_txrx_msg(controller, m->spi, tx_buf,
			tx_bits, rx_buf, rx_bits, t_bits_per_word);
	return status;
}
