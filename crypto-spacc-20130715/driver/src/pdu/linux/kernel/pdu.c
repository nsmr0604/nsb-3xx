...
{
   return dma_alloc_coherent(NULL, bytes, phys, GFP_KERNEL);
}

/* free coherent memory */
void pdu_dma_free(size_t bytes, void *virt, PDU_DMA_ADDR_T phys)
{
   dma_free_coherent(NULL, bytes, virt, phys);
}
...
{
   ddt_pool = dma_pool_create("elpddt", NULL, (PDU_MAX_DDT+1)*8, 8, 0); // max of 64 DDT entries
   if (!ddt_pool) {
      return -1;
   }
#if PDU_MAX_DDT > 16
   ddt16_pool = dma_pool_create("elpddt16", NULL, (16+1)*8, 8, 0); // max of 16 DDT entries
   if (!ddt16_pool) {
   ...
   ddt16_pool = ddt_pool;
#endif
   ddt4_pool = dma_pool_create("elpddt4", NULL, (4+1)*8, 8, 0); // max of 4 DDT entries
   if (!ddt4_pool) {
      dma_pool_destroy(ddt_pool);
#if PDU_MAX_DDT > 16
...
{
   if (debug_on) {
      printk("DDT[%p]: 0x%08lx size %lu\n", ddt, (unsigned long)phys, size);
   }
...
{
   dma_sync_single_for_cpu(NULL, addr, size, DMA_BIDIRECTIONAL);
}
