...
void buildIfVc() {
     ...
     */
    {
	    ...
	    // Network address should be remote address in stead of local address
            if (0x10d1 == IfDescEp->Flags)    // when pppx
            {
                my_log(LOG_DEBUG, 0, "Before ioctl(SIOCGIFDSTADDR), IfDescEp->Flags=0x%x, addr=0x%x, subnet=0x%x\n", IfDescEp->Flags, addr, subnet);
                ...
                my_log(LOG_DEBUG, 0, "After ioctl(SIOCGIFDSTADDR), IfDescEp->Flags=0x%x, addr=0x%x, subnet=0x%x\n", IfDescEp->Flags, addr, subnet);
            }
