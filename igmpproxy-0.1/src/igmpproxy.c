...
#ifdef CONFIG_CS75XX_DATA_PLANE_MULTICAST
extern unsigned char csGetPortId(char *pdevName, cs_port_id_t *port_id);
#endif
...
#ifdef CONFIG_CS75XX_DATA_PLANE_MULTICAST
{
                        ...
                        // Initialize Cortina Multicast data-plane APIs WAN port
                	    status = csGetPortId(Dp->Name, &port_id);
                        if ((status == 0) || (port_id <= 2)) {
                            cs_l2_mcast_wan_port_id_set(0, port_id);
                        } else {
                            my_log(LOG_ERR, 0, "Vif #%d can't set as upstream.",
                                upStreamVif, Ix);
                        }
}
#endif
...
void igmpProxyCleanUp() {
    ...
    cs_port_id_t port_id;
    unsigned char status;
    /* Clear all multicast data-plane hashs */
    cs_l2_mcast_member_all_clear(0);
    // Initialize Cortina Multicast data-plane APIs WAN port
    cs_l2_mcast_wan_port_id_set(0, CS_DEFAULT_WAN_PORT_ID);
#endif
#ifdef CONFIG_LYNXE_KERNEL_HOOK
    /* Clear all multicast data-plane hashs */
    cs_l3_mcast_member_delete_all(0);
#endif
