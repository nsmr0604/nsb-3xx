...
#if 0
int
inotify_remove_tv_channels()
{
	char **result;
	sqlite_int64 detailID = 0;
	int rows, i, ret = 1;

	if( (sql_get_table(db, "SELECT ID from DETAILS where PATH LIKE '%.livetv'", &result, &rows, NULL) == SQLITE_OK) )
	{
		if( rows )
		{
			for( i=1; i <= rows; i++ )
...
start_inotify()
{
	...
	//cortina add
	time_t time_scan_tv = time(NULL);
	time_t scan_period = 3000;
	...
	//cortina add
	DPRINTF(E_TS_DBG_2, L_INOTIFY, "start inotify\n");
	...
	while( !quitting )
	{
		...
		if(GETFLAG(LIVETV_MASK) && (time(NULL) > time_scan_tv + scan_period))
		{
			if(pChInfo = cs_ts_channel_scan())
			{
				//inotify_remove_tv_channels();
				i=0;
				while((pChInfo+i)->pmt_pid)
				{
					insert_file((pChInfo+i)->prog_short_name, (pChInfo+i)->prog_path, "", i);
					i++;
				}
				//shmdt(chInfo);
				DPRINTF(E_TS_DBG_1, L_INOTIFY,	"inster %d channel\n", i);
			}
			else
				DPRINTF(E_ERROR, L_INOTIFY, "failed to get channel info.\n");
			time(&time_scan_tv);
		}
