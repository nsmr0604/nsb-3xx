...
static char demux_devname[CS_TS_DEV_NAME_LEN]="/dev/dvb/adapter0/demux6";
int system_time = -1;
...
static int long_timeout=1;
static int current_tp_only=1; //cortina add
static int fixed_tuner=1; //cortina add
static int get_other_nits;
static int vdr_dump_provider;
static int vdr_dump_channum;
static int no_ATSC_PSIP;
static int ATSC_type=2;
static int ca_select = -1;
static int serv_select = 1; //only video service
...
static struct service *find_service_by_source_id(struct transponder *tp, int source_id)
{
	struct list_head *pos;
	struct service *s;
				     ...
				     struct service *s)
{
	...
	for (i = 0; i < len / sizeof(s->ca_id[0]); i++)
		DPRINTF(E_TS_DBG_2, L_TSSCAN, "  CA ID 0x%04x\n", s->ca_id[i]);
}
					     ...
					     struct transponder *t)
{
	...
	t->param.inversion = spectral_inversion;
		...
		if (t->scan_done)
			DPRINTF(E_TS_DBG_3, L_TSSCAN, " (done)\n");
		if (t->last_tuning_failed)
			DPRINTF(E_TS_DBG_3, L_TSSCAN, "(C: tuning failed)\n");
}
						 ...
						 struct transponder *t)
{
	...
	t->we_flag = buf[8] >> 7;
		...
		if (t->scan_done)
			DPRINTF(E_TS_DBG_3, L_TSSCAN, " (done)\n");
		if (t->last_tuning_failed)
			DPRINTF(E_TS_DBG_3, L_TSSCAN, "(S: tuning failed)\n");
}
						   ...
						   struct transponder *t)
{
	...
	t->other_frequency_flag = (buf[8] & 0x01);
		...
		if (t->scan_done)
			DPRINTF(E_TS_DBG_3, L_TSSCAN, " (done)\n");
		if (t->last_tuning_failed)
			DPRINTF(E_TS_DBG_3, L_TSSCAN, "(T: tuning failed)\n");
}
...
static void parse_pmt (const unsigned char *buf, int section_length, int service_id)
{
	...
	while (section_length >= 5) {
		...
		case 0x1b: /* H.264 video stream */
			DPRINTF(E_TS_DBG_2, L_TSSCAN, "  VIDEO     : PID 0x%04x\n", elementary_pid);
			if (s->video_pid == 0)
		...
		case 0x04:
			DPRINTF(E_TS_DBG_2, L_TSSCAN, "  AUDIO     : PID 0x%04x\n", elementary_pid);
			...
			break;
		case 0x07:
			DPRINTF(E_TS_DBG_2, L_TSSCAN, "  MHEG      : PID 0x%04x\n", elementary_pid);
			break;
		case 0x0B:
			DPRINTF(E_TS_DBG_2, L_TSSCAN, "  DSM-CC    : PID 0x%04x\n", elementary_pid);
			break;
		case 0x06:
			if (find_descriptor(0x56, buf + 5, ES_info_len, NULL, NULL)) {
				DPRINTF(E_TS_DBG_2, L_TSSCAN, "  TELETEXT  : PID 0x%04x\n", elementary_pid);
				...
				DPRINTF(E_TS_DBG_2, L_TSSCAN, "  SUBTITLING: PID 0x%04x\n", elementary_pid);
				s->subtitling_pid = elementary_pid;
				break;
			}
			else if (find_descriptor(0x6a, buf + 5, ES_info_len, NULL, NULL)) {
				DPRINTF(E_TS_DBG_2, L_TSSCAN, "  AC3       : PID 0x%04x\n", elementary_pid);
				s->ac3_pid = elementary_pid;
				break;
			}
			/* fall through */
		default:
			DPRINTF(E_TS_DBG_2, L_TSSCAN, "  OTHER     : PID 0x%04x TYPE 0x%02x\n", elementary_pid, buf[0]);
		};
	...
	if (s->audio_num > AUDIO_CHAN_MAX) {
		...
		DPRINTF("0x%04x 0x%04x: %s -- %s, pmt_pid 0x%04x, vpid 0x%04x, apid %s\n",
	    s->transport_stream_id,
	    s->service_id,
	    s->provider_name, s->service_name,
	...
	for (i=0; i < d.number_elements; i++) {
		...
		switch (e.stream_type) {
			...
			case 0x81: /* ATSC audio */
				...
				DPRINTF(E_TS_DBG_2, L_TSSCAN, "  AUDIO     : PID 0x%04x lang: %s\n",e.elementary_PID,s->audio_lang[s->audio_num-1]);
...
/*	int protocol_version = buf[0];*/
	...
	for (i = 0; i < num_channels_in_section; i++) {
		...
		s->source_id = ch.source_id; //cortina add
...
static void parse_mgt (const unsigned char *buf, int section_length, int transport_stream_id)
{
	int tables_defined;
	int table_type;
	int table_type_PID;
	...
	buf++;
	tables_defined = buf[0] << 8 | buf[1];
	buf += 2;

	DPRINTF(E_TS_DBG_2, L_TSSCAN, "%s: tables_defined in MGT: %d\n", __func__, tables_defined);

	while(tables_defined--)
	{
		table_type = buf[0] << 8 | buf[1];	
		table_type_PID = ((buf[2] & 0x1f) << 8) | buf[3];	
		table_type_descriptors_length = ((buf[9] & 0x0f) << 8) | buf[10];
		buf += (table_type_descriptors_length + 11);
		DPRINTF(E_TS_DBG_2, L_TSSCAN, "%s: type 0x%04x PID 0x%04x descriptor length %d\n", __func__, table_type, table_type_PID, table_type_descriptors_length);
    	if(table_type == 0x0100) // EIT-0
    	{
			struct list_head *pos;
			struct service *s;
			...
			list_for_each(pos, &current_tp->services)
			{
				...
				if (s->priv)
					free(s->priv);
				s->priv = malloc(sizeof(struct section_buf));
				setup_filter(s->priv, demux_devname, table_type_PID, 0xcb, s->source_id, 1, 0, 5); /* EIT-0 */
				add_filter (s->priv);
			}
			break;
    	}
...
static void parse_multiple_string_structure(const unsigned char *buf, unsigned char *program_name, int length)
{
	int number_strings, number_segments, compression_type, mode, number_bytes;
	unsigned char iso_639_language_code[4];
	number_strings = buf[0];
	buf++;
	strncpy(iso_639_language_code, buf, 3);
	iso_639_language_code[3] = '\0';
	buf += 3;
	number_segments = buf[0];
	buf++;
	while(number_segments--)
	{
		...
		number_bytes = buf[0];
		buf++;
		strncpy(program_name, buf, number_bytes);
		program_name[number_bytes] = '\0';
		DPRINTF(E_TS_DBG_2, L_TSSCAN, "%s: program_name %s\n", __func__, program_name);
	}
	
}

static void parse_eit (const unsigned char *buf, int section_length, int source_id)
{
	int num_events_in_section;
	int event_id, start_time, title_length, descriptors_length;
	struct service *s;

	s = find_service_by_source_id(current_tp, source_id);
	if(s == NULL)
	{
		DPRINTF(E_TS_DBG_2, L_TSSCAN, "%s: no service found with source id 0x%04x\n", __func__, source_id);
		return;
	}
		
	num_events_in_section = buf[1];
	buf += 2;
	while(num_events_in_section--)
	{
		event_id = ((buf[0] & 0x3f) << 8) | buf[1];	
		start_time = (buf[2] << 24) | (buf[3] << 16) | (buf[4] << 8) | buf[5];
		title_length = buf[9];
		DPRINTF(E_TS_DBG_2, L_TSSCAN, "%s: event_id 0x%04x start_time %d system_time %d\n", __func__, event_id, start_time, system_time);
		if(system_time > start_time)
			parse_multiple_string_structure(buf+10, s->current_program_name, title_length);
		buf += (title_length + 10);
		descriptors_length = ((buf[0] & 0x0f) << 8) | buf[1];
		buf += (descriptors_length + 2);
	}
}

static void parse_stt (const unsigned char *buf, int section_length, int table_id_extension)
{
	system_time = (buf[1] << 24) | (buf[2] << 16) | (buf[3] << 8) | buf[4];
  	DPRINTF(E_TS_DBG_2, L_TSSCAN, "%s: system time %d\n", __func__, system_time);
}
...
static int parse_section (struct section_buf *s)
{
	...
	if (!get_bit(s->section_done, section_number)) {
		...
		switch (table_id) {
		case 0x00:
			DPRINTF(E_TS_DBG_2, L_TSSCAN, "found PAT\n");
			parse_pat (buf, section_length, table_id_ext);
			break;

		case 0x02:
			DPRINTF(E_TS_DBG_2, L_TSSCAN, "found PMT 0x%04x for service 0x%04x\n", s->pid, table_id_ext);
			parse_pmt (buf, section_length, table_id_ext);
			break;

		case 0x41:
		case 0x40:
			DPRINTF(E_TS_DBG_2, L_TSSCAN, "found NIT (%s TS)\n", table_id == 0x40 ? "actual":"other");
			parse_nit (buf, section_length, table_id_ext);
			break;

		case 0x42:
		case 0x46:
			DPRINTF(E_TS_DBG_2, L_TSSCAN, "found SDT (%s TS)\n", table_id == 0x42 ? "actual":"other");
			parse_sdt (buf, section_length, table_id_ext);
			break;

		case 0xc8:
		case 0xc9:
			DPRINTF(E_TS_DBG_2, L_TSSCAN, "found ATSC VCT\n");
			parse_psip_vct(buf, section_length, table_id, table_id_ext);
			break;
		case 0xc7:
			DPRINTF(E_TS_DBG_2, L_TSSCAN, "found ATSC MGT\n");
			parse_mgt(buf, section_length, table_id_ext);
			break;
		case 0xcb:
			DPRINTF(E_TS_DBG_2, L_TSSCAN, "found ATSC EIT source_id 0x%04x\n", table_id_ext);
			parse_eit(buf, section_length, table_id_ext);
			break;
		case 0xcd:
			DPRINTF(E_TS_DBG_2, L_TSSCAN, "found ATSC STT\n");
			parse_stt(buf, section_length, table_id_ext);
			break;
		default:
			;
		};
...
static void setup_filter (struct section_buf* s, const char *dmx_devname,
			  ...
			  int run_once, int segmented, int timeout)
{
  DPRINTF(E_TS_DBG_2, L_TSSCAN, "%s: PID 0x%04x TID 0x%04x\n", __func__, pid, tid);
		...
		if (done || time(NULL) > s->start_time + s->timeout) {
			if (s->run_once) {
				if (done)
					...
					warning("filter timeout pid 0x%04x tid 0x%04x time %d start_time %d timeout %d\n", s->pid, s->table_id, time(NULL), s->start_time, s->timeout);
				remove_filter (s);
			}
		}
	}
...
static int __tune_to_transponder (int frontend_fd, struct transponder *t)
{
	...
	memcpy (&p, &t->param, sizeof(struct dvb_frontend_parameters));
		...
		DPRINTF(E_TS_DBG_3, L_TSSCAN, ">>> tune to: \n");
		if(log_debug_level >= E_TS_DBG_3)
			dump_dvb_parameters (get_log_fp(), t);
		if (t->last_tuning_failed)
			DPRINTF(E_TS_DBG_3, L_TSSCAN, "(tuning failed)\n");	
	...
	if (t->type == FE_QPSK) {
		...
		DPRINTF(E_TS_DBG_3, L_TSSCAN, "DVB-S IF freq is %d\n",p.frequency);
	}

	if (ioctl(frontend_fd, FE_SET_FRONTEND, &p) == -1) {
		errorn("Setting frontend parameters failed to fd %d\n", frontend_fd);
		return -2;
	}
	...
	for (i = 0; i < 20; i++) {
		...
		DPRINTF(E_TS_DBG_3, L_TSSCAN, ">>> tuning status == 0x%02x\n", s);
	...
	warning(">>> tuning failed at frequency %d!!!\n", p.frequency);
...
{
	struct section_buf s0,s1,s2,s3,s4;
	...
	if (no_ATSC_PSIP) {
		...
		if (ATSC_type & 0x1) {
			system_time = -1;
			setup_filter(&s3, demux_devname, 0x1ffb, 0xcd, -1, 1, 0, 5); /* STT */
			add_filter(&s3);
	...
	if (!no_ATSC_PSIP && (ATSC_type & 0x1)) {
			setup_filter(&s4, demux_devname, 0x1ffb, 0xc7, -1, 1, 0, 5); /* MGT */
			add_filter(&s4);
	do {
		read_filters ();
	} while (!(list_empty(&running_filters) &&
...
void scan_network (int frontend_fd, const char *initial)
{
...
//cortina add
CS_TS_CH_INFO *cs_ts_alloc_ch_info()
{
	int shmId;
	key_t key;
	CS_TS_CH_INFO *pChInfo;
	if((key=ftok(FTOK_NAME, FTOK_ID_CH))<0)
	{
		DPRINTF(E_ERROR, L_TSSCAN, "failed to get key from ftok\n");
		return NULL;
	}
	if((shmId=shmget(key,NBR_CHANNEL_INFO*sizeof(CS_TS_CH_INFO),IPC_CREAT|0666))<0)
	{
		DPRINTF(E_ERROR, L_TSSCAN, "failed to get shared memory\n");
		return NULL;
	}
	
	if((pChInfo=(CS_TS_CH_INFO*)shmat(shmId,0,0))<0)
	{
		DPRINTF(E_ERROR, L_TSSCAN, "failed to attach shared memory, id %d\n", shmId);
		return NULL;
	}
	DPRINTF(E_TS_DBG_3, L_TSSCAN, "get shared memory, id %d, addr 0x%08x\n", shmId, pChInfo);
	return pChInfo;
	
}

CS_TS_CH_INFO *cs_ts_get_ch_info()
{
	int shmId;
	key_t key;
	CS_TS_CH_INFO *pChInfo;
	...
	if((shmId=shmget(key,sizeof(CS_TS_CH_INFO),0666))<0)
	{
		DPRINTF(E_ERROR, L_TSSCAN, "failed to get shared memory\n");
		return NULL;
	}
...
int cs_ts_set_tv_channel(char *filePath)
{
	int fd,fdDvr, dvrIdx;
	int i;
	int timeout = 15;
	...
	CS_TS_CH_INFO *pChInfo;
	unsigned int tsId, pmtPid;
	char *token;
	char tmpPath[32];

	DPRINTF(E_TS_DBG_2, L_TSSCAN, "select channel request %s\n", filePath);

	pDevInfo = cs_ts_get_tv_dev_info();
	if(!pDevInfo)
		return -1;
	pChInfo = cs_ts_get_ch_info();
	if(!pChInfo)
		return -1;
	...
	sscanf(token,"%x",&tsId);
	token = strtok(NULL, ".");
	sscanf(token,"%x",&pmtPid);
	
	while(pChInfo->pmt_pid)
	{
		if((pChInfo->pmt_pid == pmtPid) && (pChInfo->ts_pid == tsId))
			break;
		pChInfo++;
	}
	if(!(pChInfo->pmt_pid))
	{
		DPRINTF(E_WARN, L_HTTP, "channel not found, pmt pid 0x%04x\n", pmtPid);
		return -1;	
	}

  dvrIdx = -1;
	for(i=0;i<CS_TS_MAX_DEMUX_DEVICE;i++)
	//for(i=CS_TS_MAX_DEMUX_DEVICE-1;i>=0;i--)
	{
		if(pDevInfo->fd_dvr[i] == 0)
		{
			dvrIdx = i;
			DPRINTF(E_TS_DBG_2, L_TSSCAN, "available dvr index %d\n", dvrIdx);
			break;
		}
	}
  if(dvrIdx < 0)
  {
		DPRINTF(E_WARN, L_TSSCAN, "dvr fd not available %s\n", filePath);
		return -1;
  }
  
  
#ifdef G2_SPEC_DEMUX
	if ((fd = open(pDevInfo->demux_data_dev[dvrIdx], O_RDWR)) < 0)
		return -4;
	g2_ts_flush_all_pid(dvrIdx);
	fdDvr = fd;
	ioctl(fdDvr, DMX_SET_BUFFER_SIZE, (188*1024));
#else
	while(timeout-- && ((fdDvr = open(pDevInfo->dvr_dev[dvrIdx], O_RDONLY)) < 0))
		usleep(200000);
#endif	
	if(fdDvr < 0)
	{
...
#ifdef CFG_FRONTEND_DEV	
	if (ioctl(pDevInfo->fd_frontend[dvrIdx], FE_SET_FRONTEND, &(pChInfo->tuner_param)) == -1) {
		errorn("Setting frontend parameters failed to fd %d\n", pDevInfo->fd_frontend[dvrIdx]);
		return -3;
	}
#endif
	...
	if ((fd = open(pDevInfo->demux_data_dev[dvrIdx], O_RDWR)) < 0)
		return -4;
#endif	
	if (set_pesfilter (fd, 0, DMX_PES_OTHER, 1) < 0)
		return -5;
	...
	if ((fd = open(pDevInfo->demux_data_dev[dvrIdx], O_RDWR)) < 0)
		return -6;
#endif	
	if (set_pesfilter (fd, pChInfo->pmt_pid, DMX_PES_OTHER, 1) < 0)
		return -7;
#ifndef G2_SPEC_DEMUX
	if ((fd = open(pDevInfo->demux_data_dev[dvrIdx], O_RDWR)) < 0)
		return -8;
#endif	
	if (set_pesfilter (fd, pChInfo->video_pid, DMX_PES_OTHER, 1) < 0)
		return -9;
	for(i=0;i<AUDIO_CHAN_MAX;i++)
	{
		if (pChInfo->audio_pid[i])
		{
#ifndef G2_SPEC_DEMUX
			if ((fd = open(pDevInfo->demux_data_dev[dvrIdx], O_RDWR)) < 0)
				return (-10-2*i);
#endif	
			if (set_pesfilter (fd, pChInfo->audio_pid[i], DMX_PES_OTHER, 1) < 0)
				return (-11-2*i);
		}
	}
	DPRINTF(E_TS_DBG_1, L_TSSCAN, "select channel %s tsid 0x%04x pmt 0x%04x video 0x%04x\n", filePath, pChInfo->ts_pid, pChInfo->pmt_pid, pChInfo->video_pid);
	//shmdt(pChInfo);
	//shmdt(pDevInfo);
#ifdef G2_SPEC_DEMUX
	pDevInfo->fd_dvr[dvrIdx] = fd;
	return fd;
#else	
	pDevInfo->fd_dvr[dvrIdx] = fdDvr;
	return fdDvr;
#endif
}
...
int cs_ts_release_tv_channel(int fd)
{
  ...
  if(fd < 0)
    return 0;
	DPRINTF(E_TS_DBG_2, L_TSSCAN, "release channel demux %d\n", fd);
  ioctl(fd, DMX_STOP);	
  close(fd);
	pDevInfo = cs_ts_get_tv_dev_info();
	if(!pDevInfo)
...
CS_TS_CH_INFO *cs_ts_channel_scan()
{
	struct list_head *p1, *p2;
	struct transponder *t;
	struct service *s;
	...
	CS_TS_CH_INFO *pChInfo;
	CS_TS_TV_DEV_INFO *pDevInfo;
	static int frontend_fd = NULL;

	//cortina add
	DPRINTF(E_TS_DBG_1, L_TSSCAN, "start scan channel\n");

#ifndef CFG_FRONTEND_DEV
	#ifdef SYSTEM_TYPE_DVB
		fe_info.type = FE_QAM;
	#endif
	#ifdef SYSTEM_TYPE_ATSC
		fe_info.type = FE_ATSC;
	#endif
#endif
	if (fixed_tuner)
	{
		current_tp = find_transponder(0);
		if (!current_tp)
	...
	else
	{
		if(frontend_fd == NULL)
		{
			pDevInfo = cs_ts_get_tv_dev_info(); //cortina add	
			if(pDevInfo)
			{
				frontend_fd = pDevInfo->fd_frontend[WORKING_DEMUX_ID];
				if(frontend_fd <= 0)
				{
					DPRINTF(E_ERROR, L_TSSCAN,  "Failed to get tuner fd.\n");	
					return -1;
				}
			}
			else
			{
				DPRINTF(E_ERROR, L_TSSCAN,  "Failed to get TV device info.\n");
				return -1;
			}
		}
		scan_network (frontend_fd, "/etc/channels.conf");
		//close(frontend_fd);
	}
#ifdef G2_SPEC_DEMUX
	...
	if(pChInfo)
		free(pChInfo);
	pChInfo = calloc(n+1, sizeof(CS_TS_CH_INFO));	
	*/
	pChInfo = cs_ts_get_ch_info();
	if(!pChInfo)
	{
		info("Error allocate shared memory for ch info.\n");
		return -1;
	}
	DPRINTF(E_TS_DBG_1, L_TSSCAN, "===== Scanned channels =====\n",n);
	DPRINTF(E_TS_DBG_1, L_TSSCAN, " TSID  PMTPID VDOPID NAME\n",n);
	n = 0;
	list_for_each(p1, &scanned_transponders)
	{
		t = list_entry(p1, struct transponder, list);
		...
		list_for_each(p2, &t->services)
		{
			...
			(pChInfo+n)->ts_pid = s->transport_stream_id;
			(pChInfo+n)->pmt_pid = s->pmt_pid;
			sprintf((pChInfo+n)->prog_path,"%04x.%04x.livetv", s->transport_stream_id, s->pmt_pid);
			(pChInfo+n)->video_pid = s->video_pid;


			for(i=0;i<s->audio_num;i++)
				(pChInfo+n)->audio_pid[i] = s->audio_pid[i];
			memcpy (&((pChInfo+n)->tuner_param), &t->param, sizeof(struct dvb_frontend_parameters));						

			if(s->current_program_name)
				sprintf((pChInfo+n)->prog_short_name, "%s - %s", s->service_name, s->current_program_name);
			else
				sprintf((pChInfo+n)->prog_short_name, "%s", s->service_name);

 			DPRINTF(E_TS_DBG_1, L_INOTIFY,	"0x%04x 0x%04x 0x%04x %s\n", (pChInfo+n)->ts_pid, (pChInfo+n)->pmt_pid, (pChInfo+n)->video_pid, (pChInfo+n)->prog_short_name);

			//vincent debug, need deal with big 5 code
			//sprintf((pChInfo+n)->prog_short_name, "Channel - %d.livetv", n);
			
			n++;
		}
	}
	
	DPRINTF(E_TS_DBG_1, L_TSSCAN, "===== End of channel lists =====\n",n);
	return pChInfo;
}

CS_TS_TV_DEV_INFO *cs_ts_get_tv_dev_info()
{
	int shmId;
	key_t key;
	CS_TS_TV_DEV_INFO *pDevInfo;
	if((key=ftok(FTOK_NAME, FTOK_ID_TV))<0)
	{
		DPRINTF(E_ERROR, L_TSSCAN, "failed to get key from ftok\n");
		return -1;
	}
	if((shmId=shmget(key,sizeof(CS_TS_TV_DEV_INFO),IPC_CREAT|0666))<0)
	{
		DPRINTF(E_ERROR, L_TSSCAN, "failed to get shared memory\n");
		return -1;
	}
	if((pDevInfo=(CS_TS_TV_DEV_INFO*)shmat(shmId,0,0))<0)
	{
		DPRINTF(E_ERROR, L_TSSCAN, "failed to attach shared memory, id %d\n", shmId);
		return -1;
	}
...
void cs_ts_show_debug_info(int level)
{
	int i;
	...
	switch(level)
	{
		...
		default:	
			...
			for(i=0;i<CS_TS_MAX_DEMUX_DEVICE;i++)
			{
				DPRINTF(E_TS_DBG_1, L_TSSCAN, "Good count %d: %d\n", i, g2_ts_get_statistic(i));
				g2_ts_clear_statistic(i);
			}
			DPRINTF(E_TS_DBG_1, L_TSSCAN, "Reset good count\n");
#endif			
		break;	
		case E_TS_DBG_2:
...
{	
	...
	if(!pDevInfo)
		return -1;
	pDevInfo->pChInfo = cs_ts_alloc_ch_info();
	if(!(pDevInfo->pChInfo))
		return -1;
	for (i=0; i<(CS_TS_MAX_DEMUX_DEVICE); i++)
	{
		sprintf(pDevInfo->frontend_dev[i], "/dev/dvb/adapter%i/frontend%i", 0, CS_TS_DEMUX_DATA_DEV_BEGIN + i);
		sprintf(pDevInfo->demux_data_dev[i], "/dev/dvb/adapter%i/demux%i", 0, CS_TS_DEMUX_DATA_DEV_BEGIN + i);
		sprintf(pDevInfo->demux_ctrl_dev[i], "/dev/dvb/adapter%i/demux%i", 0, CS_TS_DEMUX_CTRL_DEV_BEGIN + i);
		sprintf(pDevInfo->dvr_dev[i], "/dev/dvb/adapter%i/dvr%i", 0, CS_TS_DEMUX_DATA_DEV_BEGIN + i);
#ifdef CFG_FRONTEND_DEV		
		if ((pDevInfo->fd_frontend[i] = open(pDevInfo->frontend_dev[i], O_RDWR)) < 0)
			DPRINTF(E_ERROR, L_TSSCAN, "failed to open %s\n", pDevInfo->frontend_dev[i]);
		if (ioctl(pDevInfo->fd_frontend[i], FE_GET_INFO, &fe_info) == -1)
		{
			DPRINTF(E_ERROR, L_TSSCAN, "failed to read frontend information\n");
			pDevInfo->fd_frontend[i] = -1;
		}
...
#else
		DPRINTF(E_WARN, L_TSSCAN, "tuner will not be configured.\n");
#endif
#ifdef G2_SPEC_DEMUX
	pDevInfo->fd_ts_driver_mmap = g2_ts_mmap_init();
#endif

		
		/* vincent debug, netlink to be initialed
		//init netlink number
		demux->netlink_num = CS_NL_NETLINK_NUMBER_BEGIN + i;
		*/
	}
	shmdt(pDevInfo);
	return 1;
...
void cs_ts_dev_free()
{
	...
	DPRINTF(E_TS_DBG_2, L_TSSCAN, "Free ts device.\n");

	pDevInfo = cs_ts_get_tv_dev_info();
	if(pDevInfo)
	{
#ifdef G2_SPEC_DEMUX
	g2_ts_mmap_exit(pDevInfo->fd_ts_driver_mmap);
#endif

#ifdef CFG_FRONTEND_DEV
	for (i=0; i<CS_TS_MAX_DEMUX_DEVICE; i++)
		close(pDevInfo->fd_frontend[i]);
#endif
	shmdt(pDevInfo);
	}
	...
	if((shmId=shmget(key,sizeof(CS_TS_TV_DEV_INFO),0666))<0)
	{
		DPRINTF(E_ERROR, L_TSSCAN, "failed to get shared memory\n");
		return;
	}
