...
int switch_log_debug_level()
{
	log_debug_level++;
	if(log_debug_level > E_TS_DBG_3)
		log_debug_level = E_TS_DBG_1;
	log_err(E_WARN, L_GENERAL, __FILE__, __LINE__, "Change debug level to %s\n", level_name[log_debug_level]);
	return log_debug_level;
}

int
log_init(const char *fname, const char *debug, int dbg_level)
{
	int i;
	FILE *fp;
	short int log_level_set[L_MAX];

	log_debug_level = dbg_level;
	if (debug)
	{
		char *rhs, *lhs, *p;
		int n;
...
log_err(int level, enum _log_facility facility, char *fname, int lineno, char *fmt, ...)
{
	...
	if (level && level>log_level[facility] && level>log_debug_level)
		return;
	...
	t = time(NULL);
	tm = localtime(&t);
	fprintf(log_fp, "[%04d/%02d/%02d %02d:%02d:%02d p%d] ",
