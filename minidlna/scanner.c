...
insert_file(char * name, const char * path, const char * parentID, int object)
{
	...
	else if(is_livetv(path)) // cortina add
	{
 		orig_name = strdup(name);
		strcpy(base, LIVE_DIR_ID);
		strcpy(class, "item.videoItem");
		detailID = GetLiveTVMetadata(path, name, object);
	}
...
/* cortina add */					  
						LIVE_DIR_ID, "0", _("Live TV"),
						INTERNET_VIDEO_DIR_ID, "0", _("Internet Video"),
					0 };
	...
	{
		//cortina add
		if(((strncmp(containers[i],LIVE_DIR_ID, 1)==0) && GETFLAG(LIVETV_MASK)==0) ||
			((strncmp(containers[i],INTERNET_VIDEO_DIR_ID, 1)==0) && GETFLAG(INTERNET_VIDEO_MASK)==0))
			continue;
