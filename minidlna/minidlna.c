...
#include "scanner.h"
#include "inotify.h"
#include "csts/log.h" // cortina add
#ifdef TIVO_SUPPORT
...
sigterm(int sig)
{
	...
	if(sig == SIGUSR1)
	{
		debug_level = switch_log_debug_level();
		cs_ts_show_debug_info(debug_level);
		return;
	}
...
init(int argc, char * * argv)
{
	...
	else
	{
		for(i=0; i<num_options; i++)
		{
			...
			case ENABLE_DLNA_STRICT:
				...
				break;
			case ENABLE_LIVETV: //cortina add
				if( (strcmp(ary_options[i].value, "yes") == 0) || atoi(ary_options[i].value) )
					SETFLAG(LIVETV_MASK);
				break;
			case ENABLE_INTERNET_VIDEO: //cortina add
				if( (strcmp(ary_options[i].value, "yes") == 0) || atoi(ary_options[i].value) )
					SETFLAG(INTERNET_VIDEO_MASK);
				break;
			case DEBUG_LEVEL: //cortina add
				debug_level = atoi(ary_options[i].value);
				break;
			default:
	...
	{
		pid = getpid();
		log_init(NULL, "general,artwork,database,inotify,scanner,metadata,http,ssdp,tivo=debug", debug_level);
	}
	else
	{
#ifdef USE_DAEMON
...
#endif
		#ifdef READYNAS
		log_init("/var/log/upnp-av.log", "general,artwork,database,inotify,scanner,metadata,http,ssdp,tivo=warn", debug_level);
		#else
		if( access(db_path, F_OK) != 0 )
			make_dir(db_path, S_ISVTX|S_IRWXU|S_IRWXG|S_IRWXO);
		sprintf(real_path, "%s/minidlna.log", db_path);
		log_init(real_path, "general,artwork,database,inotify,scanner,metadata,http,ssdp,tivo=warn", debug_level);
		#endif
	}
	...
	if (sigaction(SIGUSR1, &sa, NULL)) // cortina add
	{
...
main(int argc, char * * argv)
{
	...
	if( (sql_get_int_field(db, "pragma user_version") != DB_VERSION) || GETFLAG(LIVETV_MASK)) // cortina add
	{
		if( new_db )
		{
