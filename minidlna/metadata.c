...
//cortina add
sqlite_int64
GetLiveTVMetadata(const char * path, char * name, int idx)
{
	metadata_t m;
	int ret;
	struct stat file;
	...
	int rows;
	char **result;
	sqlite_int64 detailID = -1;

	memset(&m, '\0', sizeof(m));
	asprintf(&m.duration, "9:99:99.999");
	asprintf(&m.date, "");
	asprintf(&m.channels, "2");
	asprintf(&m.bitrate, "583312");
	asprintf(&m.frequency, "48000");
	asprintf(&m.resolution, "704x480");
	asprintf(&m.artist, "");
	m.title=name;
	asprintf(&m.comment, "");
	asprintf(&m.dlna_pn, "MPEG_TS_SD_NA_ISO;DLNA.ORG_OP=01;DLNA.ORG_CI=0");
	asprintf(&m.mime, "video/mpeg");
	file.st_size = (999*1024*1024);
	file.st_mtime = 0;
...
//find if record exits
	...
	if( (sql_get_table(db, sql, &result, &rows, NULL) == SQLITE_OK) )
	{
		...
		{
			//found, update record
			ret = sql_exec(db, "UPDATE DETAILS SET TITLE = '%q' WHERE ID = '%q'", name, result[1], NULL);
			detailID = strtoll(result[1], NULL, 10);
			sqlite3_free_table(result);
		}
		else 
		{
			...
			ret = sql_exec(db, "INSERT into DETAILS"
				" (PATH, SIZE, TIMESTAMP, DURATION, DATE, CHANNELS, BITRATE, SAMPLERATE, RESOLUTION,"
				"  CREATOR, TITLE, COMMENT, DLNA_PN, MIME) "
				"VALUES"
				" (%Q, %lld, %ld, %Q, %Q, %Q, %Q, %Q, %Q, %Q, '%q', %Q, %Q, '%q');",
				path, file.st_size, file.st_mtime, m.duration,
							   m.date, m.channels,
								   m.bitrate, m.frequency, m.resolution, m.artist,
								   m.title ? m.title : name, m.comment, m.dlna_pn,
	...
	//vincent debug, todo: check for removed channel.
