...
#define MAX_BUFFER_SIZE (65535) // 64KB cortina add
...
//cortina add
void
send_livetv(struct upnphttp * h, int sendfd)
{
	off_t ret;
	unsigned char buf[188*50];

	DPRINTF(E_WARN, L_HTTP, "receive TS from %d\n", sendfd);
	while(1)
	{
		if(((ret = read(sendfd, buf, sizeof(buf))) < 0) && errno == EOVERFLOW)
			ret = read(sendfd, buf, sizeof(buf));
		if( ret == -1 )
		{
			DPRINTF(E_WARN, L_HTTP, "livetv read error :: error no. %d [%s]\n", errno, strerror(errno));
			if( errno != EAGAIN )
				break;
		}
		//ret = write(h->socket, buf, sizeof(buf));
		ret = send(h->socket, buf, sizeof(buf), 0);
		if( ret == -1 )
		{
			DPRINTF(E_WARN, L_HTTP, "livetv write error :: error no. %d [%s]\n", errno, strerror(errno));
			if( errno != EAGAIN )
				break;
		}
	}
...
SendResp_dlnafile(struct upnphttp * h, char * object)
{
	...
	DPRINTF(E_WARN, L_HTTP, "Serving DetailID: %lld [%s]\n", id, last_file.path);
	...
	//cortina add
	if(is_livetv(last_file.path))
	{
		size = (999*1024*1024);
		sendfh = cs_ts_set_tv_channel(last_file.path);
		if(!sendfh)
		{
			DPRINTF(E_WARN, L_HTTP, "failed to set tv channel %s, re: %d %s\n", last_file.path, sendfh);
			goto error;
		}
	}
	else
	...
	if( (send_data(h, header, strlen(header), MSG_MORE) == 0) && (h->req_command != EHead) && (sendfh > 0) )
	{
		if(is_livetv(last_file.path)) //cortina add
		{
			send_livetv(h, sendfh);
			cs_ts_release_tv_channel(sendfh);
		}
		else     
	...
	error:
	if(is_livetv(last_file.path) && (sendfh > 0)) //cortina add
			cs_ts_release_tv_channel(sendfh);
#if USE_FORK
	if( !newpid )
		_exit(0);
