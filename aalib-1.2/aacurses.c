...
static int curses_init(struct aa_hardware_params *p, void *none, struct aa_hardware_params *dest)
{
    int a;
    if (!__curses_is_up) {
	fflush(stdout);
...
{
    if (__resized_curses)
	curses_uninit(c), curses_init(&c->params, NULL,&c->driverparams), __resized_curses = 0;
