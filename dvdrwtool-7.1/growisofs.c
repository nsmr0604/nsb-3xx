...
#define LOCKWORD_COMPARE_AND_SWAP(lock, addr, old_val, new_val) \
	({                                      \
			 int tmp;           \
			 __asm__ __volatile__ ("         \
				 ...
				 1:              ldrex %0, [%4];             \
						 ...
						 ldmfd   sp!, {r10}; "       \
						 : "=&r" (tmp)               \
						 : "r" (addr), "r" (old_val), "r" (new_val), "r" (lock), "r"(1), "r"(0) \
						 : "cc", "memory");           \
		         })
 ...
 */
#ifdef __ARM_ARCH_7A__
static unsigned int write_cnt;
static unsigned int locked_flag = 0;
#endif
...
{ int		n,infd=(size_t)arg;
  ...
  sigset_t		set;
...
#ifdef __ARM_ARCH_7A__
    volatile unsigned int  read_cnt;
    read_cnt = 0;
#endif
    while (1)
    {	
	...
	{   off += n;
	    ...
	    {
#ifdef __ARM_ARCH_7A__
	        LOCKWORD_COMPARE_AND_SWAP( &locked_flag, &highest_ecc_block, highest_ecc_block, highest_ecc_block+1 );
		read_cnt++;
#else
		highest_ecc_block++;
#endif
...
#ifdef __ARM_ARCH_7A__
	LOCKWORD_COMPARE_AND_SWAP( &locked_flag, &write_cnt, write_cnt, write_cnt+1 );
#else
