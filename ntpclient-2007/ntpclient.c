...
{
	u32 data[12];
#ifdef CONFIG_HGU_DEBUG
	int rc = 0;
#endif /* CONFIG_HGU_DEBUG */ 
#define LI 0
#define VN 3
#define MODE 3
#define STRATUM 0
...
#ifndef CONFIG_HGU_DEBUG
	send(usd,data,48,0);
#else /* CONFIG_HGU_DEBUG */ 
	rc = send(usd,data,48,0);
	if (debug > 2) printf("send len:%d\n", rc);
#endif /* CONFIG_HGU_DEBUG */
}
...
#else /* CONFIG_HGU_DEBUG */
	if (debug) printf("Listening socket %d ...\n", usd);
#endif /* CONFIG_HGU_DEBUG */
...
#ifdef CONFIG_HGU_DEBUG
			if (debug > 2) printf("set time success\n");
			rc = 0; /* success */
#endif /* CONFIG_HGU_DEBUG */
		} else {
