...
static	int	pollfreq = 5, pollfreqalert = 5, lowbatt_percent = 5;
...
static void ups_low_batt_down(utype_t *ups)
{
	if (flag_isset(ups->status, ST_LOWBATT_DOWN)) { 	/* no change */
	...
	do_notify(ups, NOTIFY_LOWBATT_DOWN);
	setflag(&ups->status, ST_LOWBATT_DOWN);
	doshutdown();
	//setflag(&ups->status, ST_LOWBATT_DOWN);
...
{
        char batteryCharge[SMALLBUF];
	int nowBatteryCharge;
	...
	//if IS ON BATTERY mode, we need to check whether less than user's setting
	if (get_var(ups, "battery", batteryCharge, sizeof(batteryCharge)) == 0) {
		//debug("nowBatteryCharge: %s\n", batteryCharge);
		nowBatteryCharge=atoi(batteryCharge);
		//debug("nowBatteryCharge: %d\n", nowBatteryCharge);
		//debug("lowbatt_percent: %d\n", lowbatt_percent);
		if(nowBatteryCharge < lowbatt_percent){
			//debug("Need to shutdown\n");
			ups_low_batt_down(ups);
		}
	}	
...
static int parse_conf_arg(int numargs, char **arg)
{
	...
	/* LOWBATT_PERCENT <num> */
	if (!strcmp(arg[0], "LOWBATT_PERCENT")) {
		lowbatt_percent = atoi(arg[1]);
		return 1;
	}	
